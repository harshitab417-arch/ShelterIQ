...
router.post('/generate', async (req, res) => {
  try {
    ...
    if (checkIsFallback()) {
      ...
      return res.json({ message: 'Report generated successfully.', report: record, downloadUrl: `/api/reports/download/${filename}` });
    }

    const reportDoc = new Report(reportPayload);
    await reportDoc.save();
    res.json({ message: 'Report generated successfully.', report: reportDoc, downloadUrl: `/api/reports/download/${filename}` });
  } catch (err) {
